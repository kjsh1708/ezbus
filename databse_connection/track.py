...
def sendToSQL(busStopId=2, peopleNumber=0):
    ...
    # 1. Establish Connection to the database at local host (testing environment)
    try:
        mydb = mysql.connector.connect(
            host="localhost", port=3306, user="root", password="", database="ezBusDB")
    ...
    for frame_idx, (path, img, im0s, vid_cap, s) in enumerate(dataset):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                persons = ["No","detections", "0"]
                deepsort.increment_ages()
                LOGGER.info('No detections')

            count += 1
            if count == 20:
                #send data here for every 20 frame send once 
                print("Send data here",persons[2])
                sendToSQL(2, persons[2])
                count = 0
            print(count) # remove code 
